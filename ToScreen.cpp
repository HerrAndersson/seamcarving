...
HRESULT ToScreen::CompileShader(char* shaderText, char* pEntrypoint, char* pTarget, D3D10_SHADER_MACRO* pDefines, ID3DBlob** pCompiledShader)
{
	...
	HRESULT hr = D3DCompile(shaderText, strlen(shaderText), NULL, pDefines, NULL, pEntrypoint, pTarget, dwShaderFlags, NULL, pCompiledShader, &pErrorBlob);
...
HRESULT ToScreen::Init(HWND hwnd)
{
	...
	for( UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; driverTypeIndex++ )
	{
		...
		hr = D3D10CreateDeviceAndSwapChain( nullptr, mDriverType, nullptr, createDeviceFlags, D3D10_SDK_VERSION, &sd, &mSwapChain, &mDevice );
		if( SUCCEEDED( hr ) )
			break;
	}
	if( FAILED(hr) )
